...
if st.session_state.PDF_FILENAME:
    # 0. 🔍 IDENTIFICATION DES ÉQUIPES ET DU STAFF
    EQUIPE_A, EQUIPE_B = process_and_structure_noms_equipes(st.session_state.PDF_FILENAME)

    st.header(f"🏐 Match : {EQUIPE_A} 🆚 {EQUIPE_B}")

    # Zone d'information générale (Joueurs, Staff) dans des colonnes
    col_info1, col_info2 = st.columns(2)
    with col_info1:
        st.subheader("👥 Joueurs & Libéros")
        df_j = extraire_entites_df(st.session_state.PDF_FILENAME, "joueurs")
        df_l = extraire_entites_df(st.session_state.PDF_FILENAME, "liberos")
        st.dataframe(pd.concat([df_j, df_l]), use_container_width=True)

    with col_info2:
        st.subheader("👔 Staff Technique")
        st.dataframe(extraire_entites_df(st.session_state.PDF_FILENAME, "staff"), use_container_width=True)
    ...
    if raw_data_scores is not None:
        FINAL_DATAFRAME_SCORES = process_and_structure_scores(raw_data_scores)

        st.divider()
        st.subheader("📊 Récapitulatif du Match")
        st.table(FINAL_DATAFRAME_SCORES) # st.table est mieux pour les petits tableaux fixes

        # 2. CRÉATION DES ONGLETS PAR SET
        # On crée dynamiquement les onglets pour les sets joués
        sets_joues = []
        for i in range(5):
            if check_set_exists(FINAL_DATAFRAME_SCORES, i):
                sets_joues.append(f"Set {i+1}")
        ...
        if sets_joues:
            ...
            for idx, tab_name in enumerate(sets_joues):
                with tabs[idx]:
                    set_num = idx + 1
                    sc_a = FINAL_DATAFRAME_SCORES.iloc[idx, 0]
                    sc_b = FINAL_DATAFRAME_SCORES.iloc[idx, 1]

                    st.info(f"🔥 Analyse du {tab_name} (Score final: {sc_a} - {sc_b})")

                    # Logique d'extraction spécifique par Set
                    if set_num == 1:
                        df_a_raw = extract_raw_set_1_a(st.session_state.PDF_FILENAME)
                        df_b_raw = extract_raw_set_1_b(st.session_state.PDF_FILENAME)
                        df_a = process_and_structure_set_1_a(df_a_raw)
                        df_b = process_and_structure_set_1_b(df_b_raw)
                        tm = extract_temps_mort_set_1(st.session_state.PDF_FILENAME)
                    elif set_num == 2:
                        df_a_raw = extract_raw_set_2_a(st.session_state.PDF_FILENAME)
                        df_b_raw = extract_raw_set_2_b(st.session_state.PDF_FILENAME)
                        df_a = process_and_structure_set_2_a(df_a_raw)
                        df_b = process_and_structure_set_2_b(df_b_raw)
                        tm = extract_temps_mort_set_2(st.session_state.PDF_FILENAME)
                    # ... Ajoutez les conditions elif pour les sets 3, 4 et 5 sur le même modèle

                    # AFFICHAGE DES TEMPS MORTS
                    st.write(f"⏱️ **Temps Morts :** {EQUIPE_A} : `{tm[0] or '—'}` , `{tm[1] or '—'}` | {EQUIPE_B} : `{tm[2] or '—'}` , `{tm[3] or '—'}`")

                    # AFFICHAGE DU GRAPHIQUE CHRONOLOGIQUE
                    tracer_duel_equipes(df_a, df_b, titre=f"Évolution {tab_name}", nom_g=EQUIPE_A, nom_d=EQUIPE_B)

                    # AFFICHAGE DES DUELS DE ROTATION
                    st.subheader(f"🔄 Analyse des Rotations - {tab_name}")

                    # Préparation des rotations (Ligne R0)
                    ...
                    for i in range(6):
                        ...
                        # Dessin Colonne G (Service A)
                        dessiner_rotation_couleurs(axes[i, 0], EQUIPE_A, r_a[i], EQUIPE_B, r_b[i], serveur='A')
                        if m_a:
                            txt = f"pts marqués ({EQUIPE_A}) : {sum(m_a)}\npts encaissés ({EQUIPE_B}) : {sum(m_b)}\ndifférence : {sum(m_a)-sum(m_b):+d}"
                            axes[i, 0].text(9, -1.8, txt, ha='center', family='monospace', weight='bold', bbox=dict(facecolor='white', alpha=0.8))

                        # Dessin Colonne D (Service B)
                        dessiner_rotation_couleurs(axes[i, 1], EQUIPE_A, r_a[i], EQUIPE_B, r_b[i], serveur='B')
                        if m_b:
                            txt = f"pts marqués ({EQUIPE_B}) : {sum(m_b)}\npts encaissés ({EQUIPE_A}) : {sum(m_a)}\ndifférence : {sum(m_b)-sum(m_a):+d}"
                            axes[i, 1].text(9, -1.8, txt, ha='center', family='monospace', weight='bold', bbox=dict(facecolor='white', alpha=0.8))

                    st.pyplot(fig_rot)

else:
    st.warning("👈 Veuillez uploader un fichier PDF dans la barre latérale pour commencer.")
